...
**Status:** Phases 0-3 complete; Phases 4 (speech to text), 5 (text to speech), 6 (widget form), 7 (model-choice, settings depth & privacy modes), 8 (wake word & hands-free polish), and 9 (capability expansion - the first non-saple capability) implemented and all automated gates green, live spoken round-trip + hands-free check pending a manual run (needs a real mic + OpenAI key). Phase 9's headline landed: June now has a **non-saple MCP capability** - a path-contained local files server (`mcp/files/`, tools `list_files`/`read_file`/`write_file`) attached through the exact same seam as saple-bridge-control, proving "a capability is a server, not new core code" with a second, unrelated server. It is local/offline-safe (works under Strict offline), off by default (opt-in + one allowed folder in settings), and writes are gated by the §5 approval layer while reads run automatically; a live MCP-client smoke did a list/read/write round-trip and rejected a `../../etc/passwd` containment escape. The rest of Phase 9 (recovery drills, structured redacted logs, diagnostics export, accessibility, installer/auto-update, latency measurement, artemis wrap, Node-core sidecar) is the ops/hardening tail, deferred - the "does something outside saple by voice" exit clause is met; the "installable build recovers from every drill" clause is that tail. Phase 8 added hands-free activation - a wake-word listener ("Hey June", toggle + phrase + sensitivity in settings) that drives the same PTT activation, committed as STT-gated phrase spotting (a local openWakeWord ONNX model swaps in behind the same callback later) and honestly disabled under privacy modes that keep voice on-device - plus an adaptive-noise-floor refinement to the barge-in monitor for fewer false interruptions. Phase 7 added the app window's real settings surface (per-stage provider/model pickers with test buttons, keychain-backed multi-provider keys, privacy modes, diagnostics), made the brain roster genuinely swappable (Claude via the Agent SDK plus one `OpenAiCompatBrain` covering OpenAI/Gemini/Ollama/LM Studio/custom over the same execution-layer gate), and enforces the §5 privacy tiers from per-provider offline-safe metadata at both the UI and the run path. Phase 6 shipped the always-on floating widget (orb-that-expands, free-drag, frameless/opaque/on-top, per the user's spec), the on-demand full application window, cross-window session broadcast, and the interactive approval gate that replaced Phases 4/5's fail-closed stub - either window can now approve or reject the same command. Phase 3's exit criterion passed live on 2026-07-19: typing "open 5 claude agents and 4 codex agents in this workspace" into the text agent core spawned exactly 5 + 4 real agents through the approval gate, June reported started/failed/skipped accurately from bridge results, and a denied gate provably blocked a spawn (enforced in the execution layer, not the prompt). Phase 4 (speech to text) is unblocked. Remaining Phase 3 tail: the app-UI surface for the stream/approvals (the text harness proves the loop; the UI reuses the same core).
**Owner:** prabhash1889
