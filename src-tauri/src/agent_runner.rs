    ...
    std::thread::spawn(move || {
        for line in BufReader::new(stdout).lines() {
            ...
            match v.get("t").and_then(|t| t.as_str()) {
                ...
                Some("audit") => crate::ledger::append_audit(&app, &v),
                Some("blocked") => {
...
pub fn run_unattended(
    ...
    match outcome {
        ...
        Ok(Err(msg)) => {
            crate::ledger::append_run(app, turn, &source_rec, &prompt_rec, &started, &msg, true, &blocked, None);
            Err(msg)
        }
        Err(()) => {
...
mod tests {
    ...
    // The run-ledger redaction tests moved with the code to ledger.rs (2.3).
